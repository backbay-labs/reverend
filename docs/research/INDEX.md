...
| [Collaboration & Review Design](collaboration-review-design.md) | PR-like annotation review workflows, branching/merge semantics for RE databases, changeset diff rendering, conflict resolution UX, attribution schema, workflow orchestration (solo/pair/team/triage), Ghidra Server extension points, notification architecture |
| [Analysis Data Plane Spec](analysis-data-plane-spec.md) | Unified relational+graph schema (18 entity types, full DDL), query model with SQL examples, provenance linkage via receipt chains, indexing strategy (GIN/GiST/HNSW), 2-tier storage (SQLite local + PostgreSQL corpus), scale targets (10K–1M binaries), 3-phase migration plan |
| [Agent Runtime Security Spec](agent-runtime-security-spec.md) | STRIDE threat model for LLM/agent integration, capability-based permission system (12 capability groups), egress controls (DNS allowlisting, proxy), secrets handling, sandbox boundaries (Flatpak/Bubblewrap/Landlock), 5 attack simulations, 13-row threat matrix, enforcement architecture |
| [Corpus-Scale KB Architecture](corpus-kb-architecture.md) | Ingestion pipeline (extract→normalize→embed→store), 3-tier deduplication (exact/fuzzy/semantic), 4-tier cross-binary identity model, PostgreSQL+pgvector storage with full SQL schema, query patterns (kNN/identity/provenance), storage cost estimates (1K–1M binaries), latency SLAs, benchmark plan |
| [Type Lifecycle UX](type-lifecycle-ux.md) | Type state machine (inferred→accepted→propagated), multi-signal confidence model (5 signal types), conflict resolution strategies, "Type PR" approval flows, rollback semantics via Ghidra transactions, reviewer rubric, propagation rules (intra-function/cross-function/corpus), Ghidra DataTypeManager integration design |
| [Dynamic-Static Evidence Model](dynamic-static-evidence-model.md) | Evidence taxonomy (traces, taint labels, symbolic constraints, coverage maps), trace-to-code mapping via Ghidra Trace RMI, temporal query model, unified schema (15 tables, full DDL), taint/constraint evidence storage, UI overlay patterns (heatmaps, flow graphs, timeline), storage estimates and partitioning strategy |
| [Legal Compliance Playbook](legal-compliance-playbook.md) | DMCA §1201 interoperability/security research decision trees, CFAA good-faith research guidance, EU Directive 2009/24/EC, cloud vs offline deployment policy templates, audit requirements (NIST SP 800-53 mapping), export controls (EAR/Wassenaar), open-source license compliance matrix, pre-deployment checklist |
| [Evaluation Harness](evaluation-harness.md) | Cross-feature evaluation matrix (semantic search/type recovery/diffing/dynamic-static/ML/collaboration), 14 reproducible datasets, acceptance metrics with thresholds, regression suites, human productivity protocol (24-participant study design), metric dashboard schema, automated CI pipeline, benchmark reproduction guide. Repo-local implementation includes deterministic smoke metrics for similarity/type/diff via `eval/run_smoke.sh` and checksum-pinned fixtures in `datasets/datasets.lock.json`. |
| [Python Integration Ops Matrix](python-integration-ops-matrix.md) | Platform compatibility matrix (PyGhidra/Ghidrathon/Bridge × OS/Python/JDK), CI/CD patterns (GitHub Actions/GitLab YAML), headless reliability analysis, dependency reproducibility, 25+ failure mode catalog, performance benchmarks, package ecosystem compatibility (13 packages), recommended configurations per use case |
...
|---|---|---|---|
| **Receipts-first automation** | MVP | [Ecosystem](ecosystem-plugins-firmware.md), [Ghidra Internals](ghidra-internals-architecture.md), [AI-Assisted RE](ai-assisted-reverse-engineering.md), [Collaboration](collaboration-review-design.md), [Data Plane](analysis-data-plane-spec.md) | No existing plugin provides structured audit trails; GhidrAssist RLHF is partial exception. Receipts architecture designed: hash-chain audit trails, confidence scoring, rollback via Ghidra transactions. Analyzer + Plugin interfaces are the integration surface. Collaboration doc adds attribution schema and changeset review. Data plane spec provides provenance linkage DDL. |
| **Semantic search / find similar functions** | MVP | [Binary Similarity](binary-similarity-semantic-search.md), [Ghidra Internals](ghidra-internals-architecture.md), [Corpus KB](corpus-kb-architecture.md), [Evaluation](evaluation-harness.md) | VexIR2Vec is best fit for p-code architecture; BSim internals documented (GenerateSignatures pipeline, LSH indexing, 3 DB backends); layered FID → embeddings → LLM is the right design. Emerging: EBM, KEENHash, multi-modal embeddings. Corpus KB adds kNN query patterns and pgvector indexing. Evaluation harness defines R@1/R@10/MRR acceptance thresholds. |
| **Modern Python story** | MVP | [Ecosystem](ecosystem-plugins-firmware.md), [Ghidra Internals](ghidra-internals-architecture.md), [Python Ops](python-integration-ops-matrix.md) | PyGhidra (bundled, JPype) is canonical; Ghidrathon (Jep) for tighter in-process coupling. Headless CI/CD patterns documented with PyGhidra integration. Python Ops adds full platform compatibility matrix, 25+ failure modes, CI/CD YAML templates, and performance benchmarks. |
| **Corpus-scale knowledge base** | Mid-term | [Binary Similarity](binary-similarity-semantic-search.md), [Diffing](diffing-matching-deobfuscation.md), [Malware Analysis](malware-analysis-anti-analysis.md), [Corpus KB](corpus-kb-architecture.md), [Data Plane](analysis-data-plane-spec.md) | BSim + PostgreSQL already provides corpus-scale search; needs neural embedding backend and Lumina-style annotation sharing. BSim for malware family correlation documented. Corpus KB adds ingestion pipeline, 4-tier identity model, storage cost estimates (1K–1M binaries), and latency SLAs. Data plane spec provides 18-entity relational schema and 2-tier storage design. |
| **Diffing uplift** | Mid-term | [Diffing](diffing-matching-deobfuscation.md), [Ghidra Internals](ghidra-internals-architecture.md), [Vuln Discovery](vulnerability-discovery-exploit-dev.md), [Evaluation](evaluation-harness.md) | BSim Program Correlator + PatchDiff Correlator + ghidriff automation; BinExport interop via existing plugin; narrative reports via markdown output. PatchDiff-AI multi-agent pipeline for automated Patch Tuesday analysis. Evaluation harness adds diff accuracy metrics and regression suites. |
| **Type lifecycle UX** | Mid-term | [Decompilation & Type Recovery](decompilation-type-recovery.md), [Ghidra Internals](ghidra-internals-architecture.md), [Type Lifecycle](type-lifecycle-ux.md), [Evaluation](evaluation-harness.md) | BinSub (63x speedup) and RETYPD for constraint-based inference; IDIOMS for joint neural prediction; DataTypeManager API deep dive documented. Type Lifecycle adds state machine (inferred→accepted→propagated), multi-signal confidence model, "Type PR" review flows, rollback semantics, and propagation rules. |
| **Dynamic-static unification** | Long-term | [Symbolic Execution & Fuzzing](symbolic-execution-fuzzing-dynamic.md), [Ghidra Internals](ghidra-internals-architecture.md), [Vuln Discovery](vulnerability-discovery-exploit-dev.md), [Evidence Model](dynamic-static-evidence-model.md) | Trace RMI protocol supports custom connectors; p-code emulator JIT documented; SymQEMU best binary-only concolic engine; taint analysis frameworks (PANDA/Triton/angr) provide dynamic evidence; 7 concrete fusion patterns. Evidence Model adds unified schema (15 tables), temporal queries, taint/constraint storage, and UI overlay patterns. |
| **Safe ML/agent runtime** | Long-term | [AI-Assisted RE](ai-assisted-reverse-engineering.md), [Ecosystem](ecosystem-plugins-firmware.md), [Agent Security](agent-runtime-security-spec.md), [Legal](legal-compliance-playbook.md) | 8+ MCP servers compared; 3 policy modes (offline/allow-listed/full cloud); sandboxing architecture; prompt injection from binary content documented. Agent Security adds STRIDE threat model, capability-based permissions, 5 attack simulations, and enforcement architecture. Legal Playbook adds DMCA/CFAA/EU decision trees, export controls, and audit requirements. |
| **Collaboration & review** | Cross-cutting | [Collaboration](collaboration-review-design.md), [Data Plane](analysis-data-plane-spec.md), [Ghidra Internals](ghidra-internals-architecture.md) | PR-like annotation review, branching/merge for RE databases, conflict resolution UX, attribution schema, 4 workflow patterns (solo/pair/team/triage), Ghidra Server extension points. Data Plane provides entity-relationship schema backing the collaboration layer. |
| **Evaluation & benchmarking** | Cross-cutting | [Evaluation](evaluation-harness.md), [Binary Similarity](binary-similarity-semantic-search.md), [Decompilation](decompilation-type-recovery.md) | Unified evaluation framework covering all roadmap features; 14 reproducible datasets; acceptance thresholds per feature; human productivity protocol (24-participant study); CI pipeline integration; regression detection. |
| **E9 frontier implementation board (S1-S8)** | Post-RC execution | [Deep Research Report](../deep-research-report.md), [Data Plane](analysis-data-plane-spec.md), [Binary Similarity](binary-similarity-semantic-search.md), [Type Lifecycle](type-lifecycle-ux.md), [Dynamic-Static Evidence](dynamic-static-evidence-model.md), [Collaboration](collaboration-review-design.md) | Concrete execution mapping from research to beads `2001-2008` for knowledge graph, intent retrieval, evidence-backed annotation, type propagation, dynamic-static fusion, deterministic mission crews, corpus learning, and spec extraction. See `../e9-frontier-roadmap.md`. |
| **E10 productization wave (S1-S8)** | Post-E9 execution | [Deep Research Report](../deep-research-report.md), [Collaboration](collaboration-review-design.md), [Corpus KB](corpus-kb-architecture.md), [Dynamic-Static Evidence](dynamic-static-evidence-model.md), [Evaluation](evaluation-harness.md), [Ecosystem](ecosystem-plugins-firmware.md) | Productization mapping from frontier primitives to operator-visible cockpit/review workflows and non-toy benchmark lift proof via beads `2101-2108`. See `../e9-frontier-roadmap.md`. |
| **E11 SOTA expansion wave (S1-S8)** | Post-E10 execution | [Deep Research Report](../deep-research-report.md), [Diffing](diffing-matching-deobfuscation.md), [Symbolic/Fuzzing](symbolic-execution-fuzzing-dynamic.md), [Malware Analysis](malware-analysis-anti-analysis.md), [Ecosystem/Firmware](ecosystem-plugins-firmware.md), [Collaboration](collaboration-review-design.md), [Evaluation](evaluation-harness.md) | Frontier-v2 mapping from research to beads `2201-2208` for semantic diff narratives, deterministic deobfuscation assists, symbolic/fuzz evidence fusion, firmware vertical workflows, branch/merge collaboration topology, and adversarial benchmarked lift. See `../e9-frontier-roadmap.md`. |
| **E12 native SOTA core wave (S1-S8)** | Post-E11 execution | [Deep Research Report](../deep-research-report.md), [Ghidra Internals](ghidra-internals-architecture.md), [Data Plane](analysis-data-plane-spec.md), [Binary Similarity](binary-similarity-semantic-search.md), [Type Lifecycle](type-lifecycle-ux.md), [Dynamic-Static Evidence](dynamic-static-evidence-model.md), [Collaboration](collaboration-review-design.md), [Evaluation](evaluation-harness.md) | Native-depth mapping from research to beads `2301-2308` for p-code graph extraction, hybrid vector/symbolic retrieval, cockpit v2, interprocedural type propagation, dynamic timeline overlays, mission DAG execution, governance-scored pullback, and real-target lift decisioning. See `../e9-frontier-roadmap.md`. |
| **E13-E19 GA expansion program** | Post-E12 execution | [Deep Research Report](../deep-research-report.md), [Evaluation](evaluation-harness.md), [Ghidra Internals](ghidra-internals-architecture.md), [Decompiler/Type](decompilation-type-recovery.md), [Dynamic-Static Evidence](dynamic-static-evidence-model.md), [Corpus KB](corpus-kb-architecture.md), [Agent Security](agent-runtime-security-spec.md), [Binary Rewriting](binary-rewriting-transformation.md), [Vulnerability Discovery](vulnerability-discovery-exploit-dev.md) | Extended roadmap mapping to beads `2401-3006` covering real-target benchmark GA, native pluginization, decompiler-native type intelligence, dynamic/symbolic/taint evidence fabric, corpus scale hardening, zero-trust runtime controls, and patch/exploitability decision workflows. See `../e13-e19-frontier-ga-roadmap.md`. |
